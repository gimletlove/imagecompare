...
namespace {
    ...
    struct PendingComparisonResult {
        ...
        PendingComparisonResult(const PendingComparisonResult&) = delete;
        PendingComparisonResult& operator=(const PendingComparisonResult&) = delete;
        PendingComparisonResult(PendingComparisonResult&&) = delete;
        PendingComparisonResult& operator=(PendingComparisonResult&&) = delete;

        ~PendingComparisonResult() {
            if (value.success && !value.output_path.isEmpty()) {
                QFile::remove(value.output_path);
            }
        }

        ComparisonResult take() { return std::exchange(value, {}); }
    ...
    vips::VImage rgb_for_perceptual_compare(vips::VImage image) {
        ...
        if (image.interpretation() != VIPS_INTERPRETATION_sRGB) {
            try {
                ...
                // Keep the source layout; band handling below provides a fallback.
            }
        }
        if (image.bands() == 1) {
    ...
    ComparisonResult run_comparison(const ComparisonRequest& request) {
        if (request.first_image_path.isEmpty() || request.second_image_path.isEmpty()) {
            return fail(QStringLiteral("comparison request has empty image paths"));
        }

        try {
            const RenderSpec spec{.ignore_color_profile = request.display_mode == DisplayMode::StrictRaw};
            vips::VImage first = ImageSource::load_for_render(request.first_image_path, spec);
            vips::VImage second = ImageSource::load_for_render(request.second_image_path, spec);
            ...
            const QString output_path = next_output_path();
            auto output_guard = qScopeGuard([&output_path] { QFile::remove(output_path); });
            display_heatmap.pngsave(output_path.toUtf8().constData());
...
ComparisonJobQueue::ComparisonJobQueue(QObject* parent) : QObject(parent) { qRegisterMetaType<ComparisonResult>("ComparisonResult"); }
...
QUuid ComparisonJobQueue::enqueue(const ComparisonRequest& request) {
    ...
    auto task = QRunnable::create([queue_guard, job_id, request]() {
        const auto vips_thread_guard = qScopeGuard([] { vips_thread_shutdown(); });

        try {
            const auto pending_result = std::make_shared<PendingComparisonResult>();
            pending_result->value = run_comparison(request);
            if (queue_guard == nullptr) {
                return;
            }
            QMetaObject::invokeMethod(
                queue_guard,
                [queue_guard, job_id, pending_result]() {
                    if (queue_guard == nullptr) {
                        return;
                    }
